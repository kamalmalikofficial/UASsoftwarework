...
finalimg= cv2.blur(thresh1,(5,5))
_, thresh2 = cv2.threshold(finalimg, 22,255 ,cv2.THRESH_BINARY)
...
contours, _ = cv2.findContours(filter2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
# Iterate through contours


for contour in contours:
    # Approximate polygon
    ep = 0.03* cv2.arcLength(contour, True)
...
cv2.imshow('Triangle Detectn', filter2)
